...
def find_best_attack(pred_, ent_dict, rel_dict, facts, type_, dataset):
    s, p, o = pred_
    dict_s = {}
    triples = []
    nei1 = [(i[0], k, i[1]) for k, v in facts.items() for i in v if i[1] == s]
    nei2 = [(i[0], k, i[1]) for k, v in facts.items() for i in v if i[1] == o]
    e_o = ent_dict[o]
    e_s = ent_dict[s]
    if type_ == 'semantic':
        Y1 = np.dot(rel_dict[p], e_o)
        Y2 = np.dot(rel_dict[p], e_s)
    elif type_ == 'translation':
        Y1 = rel_dict[p] + e_o
        Y2 = rel_dict[p] + e_s
    if len(nei1) > 0:
        H1 = point_hess(e_o, nei1, ent_dict, rel_dict, type_)
        if len(nei1) > 50:
            nei1 = nei1[:50]
        for i in nei1:
            e1 = i[0]
            ...
            score_t, M = point_score(Y1, pred, e_o, H1, type_)
            dict_s[i] = score_t
    if len(nei2) > 0:
        H2 = point_hess(e_s, nei2, ent_dict, rel_dict, type_)
        if len(nei2) > 50:
            nei2 = nei2[:50]
        for i in nei2:
            e1 = i[0]
            ...
            if type_ == 'semantic':
                ...
                pred = rel_dict[rel] + ent_dict[e1]
            score_t, M = point_score(Y2, pred, e_s, H2, type_)
            dict_s[i] = score_t
    if nei1 and nei2:
        items = dict_s.items()
        if type(list(items)[0][1]) == float:
            return []
        if list(items)[0][1].shape != ():
            return []
        sorted_score = sorted(items, key=operator.itemgetter(1))
